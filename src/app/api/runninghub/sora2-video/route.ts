...
export async function POST(request: NextRequest) {
  ...
  console.log(`[${requestId}] ===== Sora2 Video API Request Started =====`);
  ...
  try {
    ...
    if (!aspectRatio || !['portrait', 'landscape', 'portrait-hd', 'landscape-hd'].includes(aspectRatio)) {
      ...
      nodeInfoList: [
        ...
        },
        {
          ...
          fieldData: JSON.stringify([
            { name: "portrait", index: "portrait", description: "竖版", fastIndex: 1.0, descriptionEn: "Vertical version" },
            { name: "landscape", index: "landscape", description: "横版", fastIndex: 2.0, descriptionEn: "Horizontal version" },
            { name: "portrait-hd", index: "portrait-hd", description: "竖版高清", fastIndex: 3.0, descriptionEn: "Vertical HD" },
            { name: "landscape-hd", index: "landscape-hd", description: "横版高清", fastIndex: 4.0, descriptionEn: "Horizontal HD" }
          ]),
    ...
    console.log(`[${requestId}] ===== Sora2 Video API Request Completed =====`);
    ...
    console.error('Sora2 video API error:', error);
    return NextResponse.json(
