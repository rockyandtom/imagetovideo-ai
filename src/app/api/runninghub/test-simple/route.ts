...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('简化测试 - 使用curl命令的确切参数:', JSON.stringify(data, null, 2));
    ...
    console.log('简化测试响应状态:', response.status);

    if (!response.ok) {
      const errorText = await response.text();
      console.error('简化测试失败:', response.status, errorText);
      return NextResponse.json({ 
        success: false,
        error: `Simple test failed: ${response.status}`, 
        details: errorText 
      });
    }

    const result = await response.json();
    console.log('简化测试响应:', JSON.stringify(result, null, 2));
